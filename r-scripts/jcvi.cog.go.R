...
# Example:
# download.tfs(org.code = 'pf', '186497')

download.tfs <- function(mo.taxId) {
  mo = read.delim(paste("http://www.microbesonline.org/cgi-bin/genomeInfo.cgi?taxId=",
    mo.taxId,"&cogFun=K&export=tab", sep=''),
...
download.and.save <- function(org.code, mo.taxId, target.dir='') {
  jcvi.cog.go <- download.tfs(mo.taxId)
  write.table(jcvi.cog.go, paste(target.dir, org.code,'_TF_jcvi_cog_go.txt', sep=''),
              sep='\t', row.names = F, quote=F)
}

download.and.save.names <- function(org.code, mo.taxId, target.dir='') {
  tfs <- download.tfs(mo.taxId)
  write.table(tfs[[1]], paste(target.dir, org.code, '.txt', sep=''),
