 ...
 */
var KEGG_Parser = new Object();


/**
 * This handler can scrape graphical pathway view pages. We also scrape search results
 * in a seperate handler.
 */
KEGG_Parser.recognize = function(doc) {
	if (doc) {
		...
		return url.indexOf("http://www.genome.jp/kegg-bin/mark_pathway_www") >=0 ||
			...
			url.indexOf("http://www.genome.jp/kegg-bin/show_pathway?") >=0 ||
            ...
            // this might be worth doing, but, the actual KEGG pathway is just a click away
            // || url.indexOf("http://david.abcc.ncifcrf.gov/kegg.jsp") >=0;
	}
	else
		return false;
}
 ...
 */
KEGG_Parser.getPageData = function(doc) {
    console.log("KEGG Parser parsing page...");
	...
	try {
        ...
        console.log("KEGG parser species: " + species);
        ...
        if (names != null && names.length > 0) {
            var nl = new Namelist("Genes in " + pathway + " Pathway", names.length, species, names);
        ...
        pageGaggleData.push(this.getNetworkGaggleData(doc, pathway, species));
    }
    catch (e) {
        console.log("KEGG failed to parse page: " + e);
    }
...
KEGG_Parser.getNetworkGaggleData = function(doc, pathway, species) {
 ...
 */
KEGG_Parser.getSpecies = function(doc) {
	if (doc) {
...
KEGG_Parser.getOrganismCode = function(doc) {
	elements = doc.getElementsByName("org_name");
	if (elements.length > 0)
...
KEGG_Parser.getPathwayName = function(doc) {
	try {
 ...
 */
KEGG_Parser.getNameList = function(doc) {
	if (doc) {
		...
		if (elements && elements[0]) {
			...
			dump("kegg:"+organism+ ":" + mapno + "\n");
		}

		// array of names
		var names = [];

		// tack on the ability to test for membership in the array
...
//		var reCompound = /C\d+/;
		var reGene = new RegExp(organism + ":(\\w+)");
		...
		for (var i=0; i < elements.length; i++) {
			...
			if (m) {
				...
				if (namesInQueryString[0] != "compound" && namesInQueryString[0]!="pathway") {
					for (var n in namesInQueryString) {
						m = reGene.exec(namesInQueryString[n])
						if (m) {
							var name = m[1];
 ...
 */
KEGG_Parser.get_genes_by_pathway = function(pathway)
{
	var p  = new Array();
 ...
 * want b2038
 */
KEGG_Parser._removePrefix = function(s) {
	var pos = s.indexOf(":");
	if (pos>-1)
 ...
 */
KEGG_Parser.getNetworkXml = function(organismCode, gaggleData) {
	var doc = document;
	var elements = doc.getElementsByName("mapno");
	if (elements && elements[0]) {
		var mapno = elements[0].value;
...
//		          + "/" + organismCode + mapno + ".xml";
		var url = "http://www.genome.jp/kegg-bin/download?entry=" + organismCode + mapno + "&format=kgml";
		console.log("making kegg ws request url = " + url);
		...
		console.log("Error in KEGG Parser: couldn't find mapno.\n")
	}
}
 ...
 */
KEGG_Parser.createRequestCallback = function(request, gaggleData) {
	var kegg = this;
	return function() {
		if (request.readyState == 4) {
			if (request.status == 200) {
				...
				console.log("kegg ws request completed!!");
 ...
 */
KEGG_Parser.processKeggXml = function(xml) {
	if (xml) {
		var entries = {};
		var enzymes = {};
		var relations = [];
		...
		for (var i=0; i<elements.length; i++) {
			...
			if (entry.reaction) {
				...
				console.log("ezymes[" + entry.reaction + "] = " + entry.name);
			}

			// entries can be "grouped" which seems to mean they form a complex
			if (entry.type=="group") {
				console.log("ezymes[" + entry.type + "] = " + entry.name);
				...
				console.log("entry[" + entry.type + "] = " + entry.name);
		...
		console.log("looking for groups");
			...
			for (var m=0; m<group.length-1; m++) {
				for (var n=m+1; n<group.length; n++) {
					...
					if (entries[g1] && entries[g2]) {
						console.log(">>> " + g1 + ", " + g2 + " :: " +  entries[g1].name + ", " +  entries[g2].name);
		...
		for (var i=0; i<elements.length; i++) {
			...
			console.log(" relation >>> " + entries[relation.entry1].name + ", " + entries[relation.entry2].name + " :: " +  relation.type);
		...
		for (var i=0; i<elements.length; i++) {
			...
			if (enzyme) {
				for (var j=0; j<substrates.length; j++) {
					for (var k=0; k<products.length; k++) {
						...
						console.log(">>>> " + substrate + " " + enzyme.name + " " + product);
			...
			// Seems like this shouldn't happen, but it does sometimes
				...
				console.log("KEGG XML: reaction without associated enzyme.");
				for (var j=0; j<substrates.length; j++) {
					for (var k=0; k<products.length; k++) {
						var substrate = substrates[j].getAttribute("name");
						var product = products[k].getAttribute("name");
						console.log(">>>> " + substrate + " " + product);
 ...
 */
KEGG_Parser._attributesToObject = function(element, attrs) {
	var object = {};
	for (var i in attrs) {
		var attr = attrs[i];
...
KEGG_Parser._geneNamePattern = /\w+:(\w*)/;


/**
 * strip the prefix off of KEGG node names
 */
KEGG_Parser._strip = function(s) {
	var m = this._geneNamePattern.exec(s);
	if (m)
		return m[1];
	else
		return s;
...
KEGG_Parser._stripNames = function(obj) {
	if (obj.name) {
		var s = "";
...
KEGG_Parser._requestFailed = function(request) {
	var status = "?";
	try {
		status = request.status;
	}
	catch (e) {
		console.log("failed KEGG request = " + request);
	}
 ...
 * species code
 */
KEGG_Parser.toKeggSpeciesCode = function(s) {
 ...
 */
KEGG_Parser.toStandardSpeciesName = function(code) {
